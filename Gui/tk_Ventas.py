...
def ventas():
    ...
    ventana_ventas.geometry("1000x400")
    ventana_ventas.title("Informacion relevante para el producto")
    ventana_ventas.configure(bg = "blue4")     
        
    tree_ventas = ttk.Treeview(ventana_ventas, column = ("IDD venta","Tipo de Comprobante","N° de Comprobante","Fecha","Total", "Estado","ID Cliente"), show='headings',height=4)
    ...
    tree_ventas.column("# 7", anchor=CENTER, width= 120)
    tree_ventas.heading("# 7", text="Cliente")
    ...
    ventas = db.select_all("venta","id_venta,tipo_comprobante,nro_comprobante,fecha,total,estado,id_cliente")

    for venta in ventas:
        tree_ventas.insert('','end', text="1",values=((venta[0], venta[1], venta[2], venta[3], venta[4], venta[5],venta[6])))     
